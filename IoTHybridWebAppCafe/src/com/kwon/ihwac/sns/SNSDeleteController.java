...
/**
 * Servlet implementation class SnsDeleteController
 */
...
public class SNSDeleteController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if(MemberDAO.getMdao().loginCheck(request, response)) {
			SNSDAO.getSdao().delete(request, response);
			SNSDAO.getSdao().deleteRepl2(request, response);
